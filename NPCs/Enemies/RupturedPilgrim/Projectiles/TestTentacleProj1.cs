...
namespace MythosOfMoonlight.NPCs.Enemies.RupturedPilgrim.Projectiles
{
    public class TestTentacleProj1 : ModProjectile
    {
        ...
        {
            return Projectile.Center - Main.screenPosition + (Projectile.oldRot[i] * (float)Math.Sqrt((float)(100f - i) / 100f) + Projectile.ai[0]).ToRotationVector2() * (.1f * i * (float)(1f - (float)Math.Pow(.965f, i)) / .035f);
        }
