...
{
    public class TestTentacleProj1 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                projectile.rotation += MathHelper.ToRadians((600 - projectile.timeLeft) / 360f) * (projectile.ai[1] > 0 ? 1 : -1);
            }
        ...
        {
            return projectile.Center - Main.screenPosition + (projectile.oldRot[i] * (float)Math.Sqrt((float)(100f - (float)i) / 100f) + projectile.ai[0]).ToRotationVector2() * (.1f * (float)i * (float)(1f - (float)Math.Pow(.965f, i)) / .035f);
        }
        ...
        {
            if (projectile.timeLeft > 60 && projectile.timeLeft < 510)
            {
