...
using Terraria.UI;
using static MythosOfMoonlight.Common.Graphics.Misc.Lenikya;
using static System.Net.Mime.MediaTypeNames;

namespace MythosOfMoonlight.Common.Graphics.Misc
{
    public static class Lenikya
    {
        ...
        public static void DrawLenikyaCombatText(SpriteBatch spriteBatch)
        {
            if (IsLoaded)
            {
                ...
                Alphabet = Helper.GetTex("MythosOfMoonlight/Assets/Textures/LenikyaLanguage");
                Texture2D Outline = Helper.GetTex("MythosOfMoonlight/Assets/Textures/LenikyaLanguage_Outline");
                ...
                for (int i = 0; i < strings.Length; i++)
                {
                    if (strings[i].active)
                    {
                        ...
                        {
                            int frameY = 0;
                            int letter = char.ToUpper(c) - 'A';
                            if (letter + 'A' < 'A' || letter + 'A' > 'Z')
                            {
                        ...
                        for (int j = 0; j < strings[i].text.Length; j++)
                        {
                            ...
                            int frameY = 0;
                            int letter = char.ToUpper(strings[i].text[j]) - 'A';
                            if (letter + 'A' < 'A' || letter + 'A' > 'Z')
                            {
        ...
        public static void DrawLenikyaText(this SpriteBatch spriteBatch, string text, Vector2 position, Color color, Color outlineColor, float scale = 1f, bool centered = true) //scale should remain constant.
        {
            ...
            for (int j = 0; j < text.Length; j++)
            {
                ...
                int frameY = 0;
                int letter = char.ToUpper(text[j]) - 'A';
                if (letter + 'A' < 'A' || letter + 'A' > 'Z')
                {
