    ...
    public class PurpleCometSky : CustomSky
    {
        ...
        public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
        {
            if (maxDepth >= 3.40282347E+38f && minDepth < 3.40282347E+38f)
            {
                ...
                Vector2 cometP = Vector2.Lerp(new Vector2(Main.screenWidth + 300, 100), new Vector2(-500, 400), (float)Main.time / 32400);
                ...
                spriteBatch.Draw(starTex, cometP + starOffset, null, Color.DarkViolet * Intensity * 0.1f * ((((float)Math.Sin((double)Main.GlobalTimeWrappedHourly) + 1) * 0.5f) + 0.4f + glow), Main.GameUpdateCount * 0.0035f, starTex.Size() / 2, .45f + glow * 0.1f, SpriteEffects.None, 0f);
                spriteBatch.Draw(starTex, cometP + starOffset, null, Color.DarkViolet * Intensity * 0.35f, Main.GameUpdateCount * -0.0035f, starTex.Size() / 2, .45f + glow * 0.1f, SpriteEffects.None, 0f);

                spriteBatch.Draw(starTex, cometP + starOffset, null, Color.DarkViolet * Intensity, 0, starTex.Size() / 2, .15f, SpriteEffects.None, 0f);
                spriteBatch.Draw(starTex3, cometP + starOffset, null, Color.White * 0.65f * Intensity, 0, starTex3.Size() / 2, .5f, SpriteEffects.None, 0f);

                spriteBatch.Draw(starTex2, cometP + starOffset, null, Color.DarkViolet * Intensity * 0.1f * ((((float)Math.Sin((double)Main.GlobalTimeWrappedHourly) + 1) * 0.7f) + 0.4f + glow), Main.GameUpdateCount * 0.001f, starTex2.Size() / 2, 0.32f, SpriteEffects.None, 0f);
                spriteBatch.Draw(starTex2, cometP + starOffset, null, Color.DarkViolet * Intensity * 0.1f * ((((float)Math.Sin((double)Main.GlobalTimeWrappedHourly) + 1) * 0.7f) + 0.4f + glow), Main.GameUpdateCount * -0.001f, starTex2.Size() / 2, 0.32f, SpriteEffects.None, 0f);
        ...
        void DrawTrail()
        {
            ...
            {
                Vector2 pos = Vector2.Lerp(new Vector2(Main.screenWidth + 300, 100), new Vector2(-500, 400), (float)(Main.time - i * 50) / 32400);
                Vector2 nextPos = Vector2.Lerp(new Vector2(Main.screenWidth + 300, 100), new Vector2(-500, 400), (float)(Main.time - (i + 1) * 50) / 32400);

                Main.spriteBatch.Draw(tex, pos + starOffset, null, Color.DarkViolet * (0.7f + glow * 0.1f) * Intensity, Helper.FromAToB(pos, nextPos).ToRotation(), tex.Size() / 2, 0.1f, SpriteEffects.None, 0);
            }
        }
        void DrawShine(Vector2 pos)
        {
            Texture2D tex = Helper.GetTex("MythosOfMoonlight/Textures/Extra/slash");
            Main.spriteBatch.Draw(tex, pos, null, Color.DarkViolet * (0.7f + glow * 0.1f) * Intensity, 0, tex.Size() / 2, new Vector2(0.4f - glow * 0.1f, 0.2f + glow * 0.1f), SpriteEffects.None, 0);

            Main.spriteBatch.Draw(tex, pos, null, Color.DarkViolet * (0.7f + glow * 0.1f) * Intensity, MathHelper.PiOver2, tex.Size() / 2, new Vector2(0.4f - glow * 0.1f, 0.2f + glow * 0.1f), SpriteEffects.None, 0);
            for (int k = 0; k < 3; k++)
            {
                ...
                for (float i = 0; i < max; i++)
                {
                    ...
                    for (float j = 0; j < 2; j++)
                        Main.spriteBatch.Draw(tex, pos + offset, null, Color.DarkViolet * (k == 1 ? 0.24f : 0.4f) * alpha, angle, tex.Size() / 2, new Vector2(linesGlow, alpha) * scale, SpriteEffects.None, 0);
                }
            }
        }
