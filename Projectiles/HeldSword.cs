...
namespace MythosOfMoonlight.Projectiles
{
    public abstract class HeldSword : ModProjectile
    {
        ...
        public float swingArcModifier = 1f;
        public float startRotOffset = 0f;
        public bool IgnoreDefaultBehaviour = false;
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[1] != 0)
            {
                ...
                float swingProgress = (Ease(Utils.GetLerpValue(0f, swingTime, Projectile.timeLeft)) * swingArcModifier) - (swingArcModifier - 1) * 0.5f;
                ...
                float rotation = (direction == 1 ? start + MathHelper.Pi * 3 / 2 * swingProgress : end - MathHelper.Pi * 3 / 2 * swingProgress) + startRotOffset;
        ...
        {
            Player player = Main.player[Projectile.owner];
            if (player.active && player.channel && !player.dead && !player.CCed && !player.noItems && !IgnoreDefaultBehaviour)
            {
